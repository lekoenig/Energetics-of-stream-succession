...
# Read in Powell Center data (metabolism estimates and predictors):
metab_all <- download_metab_est(in_save_dir) 

# Read in Powell Center data (model diagnostics):
...
hist(unique(metab_filter$cor.coef))
hist(unique(metab_filter$GPP_grow_median))
...
metab_filtered_sites <- bind_rows(lapply(unique(metab_filter$site_name),check_turbidity_data)) %>%
                        ...
                        filter(.,turbidity_total_days > 80 & check_turbidity_range == "TRUE") %>% 
